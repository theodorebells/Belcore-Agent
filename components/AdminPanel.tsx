...
const AdminPanel: React.FC<AdminPanelProps> = ({ submissions, onUpdate }) => {
  ...
  const handleSelect = (s: SMESubmission) => {
    ...
    // Scroll to top on mobile when a lead is selected
  ...
  return (
    <div className="grid lg:grid-cols-12 gap-8 py-4 animate-in fade-in duration-500">
      {/* Back to list button for mobile when selected */}
      {selected && (
        <button 
          onClick={() => setSelected(null)}
          className="lg:hidden flex items-center gap-2 text-xs font-black text-emerald-600 uppercase tracking-widest mb-2"
        >
          ← Back to Pipeline
        </button>
      )}
      ...
      <div className={`lg:col-span-4 space-y-6 ${selected ? 'hidden lg:block' : 'block'}`}>
        <h2 className="text-2xl sm:text-3xl font-black text-gray-900 uppercase tracking-tight">Lead Pipeline</h2>
        
        <div className="bg-white rounded-[30px] sm:rounded-[40px] border border-gray-100 overflow-hidden shadow-xl">
          <div className="max-h-[500px] lg:max-h-[700px] overflow-y-auto no-scrollbar">
            {submissions.map(s => (
              <div 
                key={s.id} 
                onClick={() => handleSelect(s)}
                className={`p-5 sm:p-6 border-b border-gray-50 cursor-pointer transition-all hover:bg-emerald-50 ${selected?.id === s.id ? 'bg-emerald-50 border-emerald-100' : ''}`}
              >
      ...
      <div className={`lg:col-span-8 ${!selected ? 'hidden lg:block' : 'block'}`}>
        {selected ? (
          <div className="bg-white p-6 sm:p-14 rounded-[30px] sm:rounded-[60px] border border-gray-100 shadow-2xl space-y-8 sm:space-y-10 animate-in slide-in-from-right-10 overflow-y-auto no-scrollbar lg:max-h-[85vh]">
            <div className="border-b pb-6 sm:pb-8">
               <p className="text-[8px] sm:text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-2">Case File: {selected.readiness.location}</p>
               <h3 className="text-3xl sm:text-4xl font-black text-gray-900 tracking-tighter">{selected.businessName}</h3>
               <div className="flex flex-wrap gap-3 sm:gap-4 mt-3">
                 <span className="text-[10px] sm:text-xs font-bold text-gray-400">Industry: {selected.industry}</span>
                 <span className="text-[10px] sm:text-xs font-bold text-gray-400">Contact: {selected.contactPerson}</span>
               </div>
            </div>

            {selected.aiStrategy && (
              <div className="bg-purple-50 p-6 sm:p-8 rounded-[30px] sm:rounded-[40px] border border-purple-100 space-y-3 sm:space-y-4">
                 ...
                 <div className="text-xs sm:text-sm font-bold text-purple-900 leading-relaxed italic whitespace-pre-wrap">
                   {selected.aiStrategy}
                 </div>
              </div>
            )}
            ...
            <div className="space-y-6 sm:space-y-8 pt-8 border-t">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                  <div className="space-y-3">
                    ...
                    <select 
                      className="w-full p-4 sm:p-5 bg-gray-50 rounded-2xl sm:rounded-3xl font-bold text-xs sm:text-sm outline-none border-2 border-transparent focus:border-emerald-500"
                      value={statusDraft}
                  ...
                  <div className="space-y-3">
                    ...
                    <input type="range" className="w-full accent-emerald-500 mt-4 h-2 rounded-full cursor-pointer" value={progressDraft} onChange={e => setProgressDraft(parseInt(e.target.value))} />
                  </div>
               </div>
               ...
               <div className="space-y-3">
                  ...
                  <textarea 
                    className="w-full p-5 sm:p-6 bg-gray-50 rounded-2xl sm:rounded-3xl text-xs sm:text-sm font-medium h-24 sm:h-32 border-2 border-transparent focus:border-emerald-500 outline-none" 
               ...
               <button 
                 onClick={handleSave}
                 className="w-full py-5 sm:py-6 bg-gray-900 text-white rounded-2xl sm:rounded-3xl font-black text-base sm:text-lg shadow-xl hover:bg-emerald-600 transition-all active:scale-95"
               >
                 Update Case File →
               </button>
            </div>
          ...
          <div className="bg-gray-50 h-full min-h-[300px] sm:min-h-[500px] rounded-[30px] sm:rounded-[60px] border-2 sm:border-4 border-dashed border-gray-200 flex flex-col items-center justify-center text-center p-8 sm:p-16">
            <span className="text-5xl sm:text-7xl mb-6 grayscale opacity-10">📁</span>
            <p className="text-lg sm:text-xl font-black text-gray-300 uppercase tracking-widest">Select a Lead to View Details</p>
          </div>
        )}
      </div>
    </div>
